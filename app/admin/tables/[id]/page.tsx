...
export default function TableDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [assignedPasses, setAssignedPasses] = useState<AssignedPass[]>([])
  ...
  const loadTableDetail = async () => {
    ...
    try {
      ...
      // Get passes assigned to this table
  ...
  const handleRemovePass = async (passId: string) => {
    if (!confirm('¿Remover esta persona de la mesa?')) return
    ...
    try {
      ...
      alert('Error al remover a la persona')
    }
  }
  ...
  const occupancy = assignedPasses.length
    ...
    <div className="min-h-screen bg-[#fbf8f0] flex flex-col overflow-hidden pb-24">
      ...
      <div className="flex flex-col items-center justify-center py-8">
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <p className="text-[#6b7566] text-sm font-medium">{occupancy} Personas confirmadas</p>
        </div>
              ...
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex flex-col truncate">
                  ...
                  <p className="text-[#6b7566] text-xs font-medium truncate">
                    De: {pass.guests?.name || 'Invitación'}
                  </p>
                </div>
              </div>
              <button 
