  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 px-4 md:px-0">
      ...
      <div className="border border-gray-200 rounded-xl p-5 flex items-center gap-4 bg-white">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-admin-sage/20 text-admin-forest">
          <DollarSign className="h-5 w-5" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-xs text-gray-500">Total Recaudado (USD)</div>
          <div className="text-2xl font-serif text-admin-forest truncate">${totalUsd.toLocaleString("en-US", { maximumFractionDigits: 2 })}</div>
      ...
      <div className="border border-gray-200 rounded-xl p-5 flex items-center gap-4 bg-white">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-admin-rose/20 text-admin-rose">
          <Coins className="h-5 w-5" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-xs text-gray-500">Total en MXN (1:20)</div>
          <div className="text-2xl font-serif text-admin-forest truncate">${totalMxn.toLocaleString("es-MX", { maximumFractionDigits: 2 })}</div>
      ...
      <div className="border border-gray-200 rounded-xl p-5 bg-white flex items-center gap-4">
        <div className="relative h-16 w-16">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-sm font-semibold text-admin-forest">{confirmedRate}%</span>
          </div>
        </div>
        <div className="flex-1">
          <div className="text-xs text-gray-500">Confirmación de asistencia</div>
          <div className="text-sm text-admin-forest">{confirmedPasses} confirmados de {totalPasses}</div>
          <div className="text-xs text-gray-500">Total invitados: {totalGuests}</div>
        </div>
        <div className="hidden md:flex h-10 w-10 items-center justify-center rounded-lg bg-admin-sage/20 text-admin-forest">
      ...
      <div className="border border-gray-200 rounded-xl p-5 bg-white flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-2xl font-serif text-admin-forest truncate">{manualReviewCount}</div>
          <div className="mt-1">
            {manualReviewCount > 0 ? (
